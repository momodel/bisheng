...
import { HeaderTitle } from "~/customizations/agentChat/components/HeaderTitle";
import { useCitationReferencePanel } from "~/components/Chat/Messages/Content/useCitationReferencePanel";
import { ExportFormatSheet, MessageSelectionToolbar, } from "~/components/Chat/MessageSelection";
import { useAuthContext, useLocalize } from "~/hooks";
import { useExitSelectionOnChatChange, useMessageSelection, type SelectableMessage, } from "~/hooks/useMessageSelection";
import usePrefersMobileLayout from "~/hooks/usePrefersMobileLayout";
import { NotificationSeverity } from "~/common";
import { useToastContext } from "~/Providers";
import { importMessagesToKnowledgeApi, listUploadableSpacesApi, } from "~/api/messageExport";
import { translateApiErrorMessage } from "~/api/request";
import { AddToKnowledgeModal, type AddToKnowledgeSelection, } from "~/pages/Subscription/Article/AddToKnowledgeModal";
import { ChatInput } from "~/customizations/agentChat/ChatInput";
import { ChatMessages } from "~/customizations/agentChat/ChatMessages";
...
import { useChatHelpers } from "~/customizations/agentChat/useChatHelpers";
import { useWebSocket } from "~/customizations/agentChat/useWebsocket";
...
export function ChatView({ data, cid, v, readOnly, isGuestMode = false }) {
    ...
    const createNewChat = useCallback(() => {
        ...
        if (cid)
            copyAppChatOrigin(cid, chatId);
        if (cid)
            copyAppChatReturnTo(cid, chatId);
    ...
    const { state: selectionState, getSelectedIds, exitSelectionMode, } = useMessageSelection();
    const isH5 = usePrefersMobileLayout();
    const { showToast } = useToastContext();
    const [exportSheetOpen, setExportSheetOpen] = useState(false);
    const [importModalOpen, setImportModalOpen] = useState(false);
    const selectableMessages = useMemo<SelectableMessage[]>(() => messages
    ...
    const handleImportSelect = useCallback(async (selection: AddToKnowledgeSelection) => {
        if (!cid)
            return;
        ...
        try {
            ...
            showToast({
                message: localize("workstation.messageExport.importSuccess") +
            ...
            showToast({
                message: translateApiErrorMessage({ status_code: e?.status_code, status_message: e?.status_message })
    ...
    const activeConversation = useMemo(() => conversations.find((item) => item.id === cid), [conversations, cid]);
    ...
    const showChatEmptyState = conversations.length === 0 &&
        messages.length === 0 &&
        !data?.guide_word &&
        !running?.inputForm &&
    ...
    return <div className="relative h-full flex flex-col">
        <div className="min-h-0 flex-1 flex flex-col bg-[position:0_100%] bg-repeat-x bg-[length:10px_432px]">
            {showChatEmptyState ? (<div className="flex min-h-0 flex-1 flex-col">
                    ...
                    <div className="relative flex min-w-0 flex-1 min-h-0 flex-col overflow-hidden">
                        ...
                        {!readOnly && (cid && selectionState.active && selectionState.chatId === cid ? (<div className="z-10 w-full max-w-[800px] mx-auto shrink-0 bg-[#fff] dark:bg-[#1B1B1B] py-1.5">
                                    <MessageSelectionToolbar chatId={cid} messages={selectableMessages} onExportToLocal={isH5 ? () => setExportSheetOpen(true) : undefined} onImportToKnowledge={() => setImportModalOpen(true)}/>
                                </div>) : (<ChatInput v={v} readOnly={readOnly}/>))}
                    </div>
        ...
        {cid && selectionState.active && selectionState.chatId === cid && (<>
                <ExportFormatSheet open={exportSheetOpen} onOpenChange={setExportSheetOpen} chatId={cid} messages={selectableMessages}/>
                <AddToKnowledgeModal open={importModalOpen} onOpenChange={setImportModalOpen} mode="channel_sync" dataSourceApi={listUploadableSpacesApi} onSyncSelect={handleImportSelect}/>
            </>)}
    </div>;
}
;
