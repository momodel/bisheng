// Frontend fork of pages/appChat/workflowAutoRerun.ts. Edit this copy for custom chat.
...
export function syncWorkflowActivation(activation: WorkflowActivation, chatId: string): WorkflowActivation {
    return activation.chatId === chatId ? activation : createWorkflowActivation(chatId);
}
export function claimWorkflowHandshake(activation: WorkflowActivation, chatId: string): boolean {
    if (activation.chatId !== chatId || activation.handshakeSent)
        return false;
    activation.handshakeSent = true;
    return true;
}
export function isWorkflowFinishedStatusCheck(data: WorkflowCloseMessage): boolean {
    if (data.category !== 'processing' || data.type !== 'close')
        return false;
...
export function decideWorkflowCloseAction({ data, activation, enabled, isStandaloneWorkflow, isNewConversation, }: WorkflowCloseDecisionInput): WorkflowCloseAction {
    if (data.category !== 'processing' || data.type !== 'close')
        return 'ignore';
    if (!isWorkflowFinishedStatusCheck(data))
        return 'manual';
    if (data.chat_id !== activation.chatId || activation.handled)
        return 'ignore';
