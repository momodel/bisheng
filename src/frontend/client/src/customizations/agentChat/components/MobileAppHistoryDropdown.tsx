...
function TruncatedLineTooltip({ text, className }: {
    text: string;
    className?: string;
}) {
    ...
    if (!truncated) {
        return (<p ref={ref} className={pClassName}>
                {text}
            </p>);
    }
...
export function MobileAppHistoryDropdown({ open, onClose, topOffset = 'calc(env(safe-area-inset-top, 0px) + 52px)', }: MobileAppHistoryDropdownProps) {
    ...
    return createPortal(<div className="fixed inset-x-0 bottom-0 z-[80] flex flex-col bg-white" style={{ top: topOffset }} role="dialog" aria-modal="true" aria-label={localize('com_app_chat_sidebar_title')}>
            ...
            <div className="shrink-0 px-3 pt-3">
                <div className="flex flex-col gap-3 rounded-md border-[0.5px] border-border-base p-2" style={{
            backgroundImage: 'linear-gradient(128.789deg, rgb(var(--brand-500)/0.04) 0%, rgb(255, 255, 255) 50%, rgb(var(--brand-500)/0.04) 100%)',
        }}>
                        ...
                        <div className="flex min-w-0 flex-1 flex-col justify-center">
                            <div className="flex items-center justify-between">
                                <h3 className="truncate text-[14px] font-medium leading-[22px]">
                                    {flowData?.name || i18n.t("com_app.custom_unknown_app")}
                                </h3>
                    ...
                    <div className="flex items-center justify-center gap-1">
                        {showShareApp ? (<button type="button" onClick={() => {
                shareApp();
            ...
            createNewChat();
            onClose();
        }} className={cn('flex h-[28px] min-w-0 items-center justify-center gap-1 rounded-md border border-border-base bg-white px-2 text-[14px] leading-[22px] transition-colors fine-pointer:hover:bg-gray-50', showShareApp ? 'flex-1' : 'w-full')}>
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-2 pb-[max(12px,env(safe-area-inset-bottom))] pt-3">
                {groups.length === 0 ? (<div className="flex flex-1 items-center justify-center px-0 py-6">
                        <p className="text-center text-[14px] leading-[19.5px] text-text-3">
                            {localize('com_app_chat_sidebar_empty')}
                        </p>
                    </div>) : (groups.map((group, groupIdx) => (<div key={groupIdx} className="flex flex-col">
