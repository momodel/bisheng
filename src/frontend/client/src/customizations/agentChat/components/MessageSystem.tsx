// Frontend fork of pages/appChat/components/MessageSystem.tsx. Edit this copy for custom chat.
...
export function MessageSystem({ title, logo, data }) {
    ...
    const logMkdown = useMemo(() => (data.thought && <Markdown content={data.thought.toString()} isLatestMessage={false} webContent={undefined}/>), [data.thought]);
        ...
        <div className="flex gap-3">
            ...
            <div className="">
                ...
                <div className={`relative rounded-2xl px-6 py-3 pb-1.5 border text-base dark:bg-gray-900 ${data.category === 'guide' ? 'bg-[#EDEFF6]' : 'bg-slate-50'} ${border[data.category || 'system']}`}>
                    {logMkdown}
                    {data.category === 'report' && <Copy className=" absolute right-4 top-2 cursor-pointer" onClick={(e) => handleCopy((e.target as HTMLElement).parentNode)}></Copy>}
                </div>
            </div>
        </div>
    </div>;
