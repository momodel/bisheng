// Frontend fork of pages/appChat/fileAcceptUtils.ts. Edit this copy for custom chat.
...
const MEDIA_EXTENSIONS = new Set(MEDIA_SUFFIXES.map((s) => s.slice(1).toLowerCase()));
export function normalizeFileAccept(value: unknown, options?: {
    mediaEnabled?: boolean;
}): UploadFileKind[] {
    const mediaEnabled = options?.mediaEnabled ?? false;
    let kinds: UploadFileKind[] = [];
    if (Array.isArray(value)) {
        kinds = value.filter((k): k is UploadFileKind => k === 'file' || k === 'image' || k === 'media');
    }
...
export function fileAcceptToInputAccept(kinds: UploadFileKind[]): string {
    ...
    if (kinds.includes('file'))
        parts.push(...FILE_SUFFIXES);
    if (kinds.includes('image'))
        parts.push(...IMAGE_SUFFIXES);
    if (kinds.includes('media'))
        parts.push(...MEDIA_SUFFIXES);
    return parts.join(',');
}
export function getFileKindByExt(ext: string): UploadFileKind {
    ...
    if (MEDIA_EXTENSIONS.has(normalized))
        return 'media';
    if (['png', 'jpg', 'jpeg', 'bmp'].includes(normalized))
        return 'image';
    return 'file';
}
